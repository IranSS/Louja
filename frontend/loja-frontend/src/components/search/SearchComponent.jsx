...
function SearchComponent() {
  ...
  useEffect(() => {
    axios
      ...
      .catch((error) => {
        console.log("Erro em pegar produtos: ", error);
      });
  }, []);
  ...
  //verificar se está logado e aparecer SnackBar
  const handleCheck = (produto) => {
    if (localStorage.getItem("token") !== null) {
      if (cartItems.some((item) => item.id === produto.id)) {
        setMessage({ message: "produto já está no carrinho", type: "error" });
      } else {
        addToCart(produto);
        setMessage({
    ...
    <div className="container-Search">
      ...
      <div className="campo-search">
        <input
          ...
          onChange={(valor) => setValueInput(valor.target.value)}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              pesquisar();
            }
